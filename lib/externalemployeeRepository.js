            ...
            request('http://172.16.1.119/imssServices/employeeServices.php?serviceName=allEmployeeDetails', function (error, response, body) {
                ...
                async.forEach(Object.keys(json.employeeDetails), function (i,callback){
                    ...
                    pool.getConnection(function(err, connection) {
                        ...
                        connection.query('call `tms`.`UPdatedate`(?, ?, ?,?, ?, ?,?, ?, ?,?,?,?,?,?,?,?)', [empId,empName,reportsToId,dateOfJoin,priorexp,dateOfBirth,imssEmail,mobile,gender,reportsToName,qualification,empStatus,designation,company,image,imssExp], function (error, results, fields) {
