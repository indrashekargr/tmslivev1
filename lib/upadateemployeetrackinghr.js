...
console.log("Connection established for updated employee project profile tracking");
    mysql.query("UPDATE `employeetrackingmaster` SET `CompanyName`=?,`ProjectName`=?,`FromDate`=? where `Id`=? AND `UserId`=?",[req.body.CompanyName,req.body.ProjectName,req.body.FromDate,req.body.Id,req.body.UserId], function(err, result) {
if (err) throw err;
